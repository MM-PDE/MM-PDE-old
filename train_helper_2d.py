...
def training_itp(itp_model: torch.nn.Module,
    ...
    for (u_base, u_super) in loader:
        ...
        loss.backward()
        losses.append(loss.detach() / batch_size / 2)
        optimizer.step()
        if optimizer2 != None:
            optimizer2.step()
...
def training_loop(model: torch.nn.Module,
    ...
    for idx, (u_base, u_super) in enumerate(loader):
        ...
        if f'{model}' == 'GNN':
            pred = model(graph)
            pred_uni = graph_creator.interpolate_pred(itp_model, pred, graph, data, device)
            labels = labels.to(device)
            loss = criterion(pred_uni, labels.reshape(-1, 1))
        else:
            pred = model(data)
